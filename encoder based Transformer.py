...
class MultiHeadAttention(nn.Module):
    ...
    MultiHeadAttention block having heads=8 number of parallel attention layers
    Each attention layer consists of scaled dot-product self-attention
    ...
    def __init__(self,embed_dim,no_heads,mask=False):
        ...
        no_heads(:obj:`int`): The number of heads.
        mask(:obj:`boolean`): Wheather mask will bw applied or not
        ...
        # breaking the dimension into chunks for each head
        self.s = embed_dim // no_heads
        
        # we must have int value as dimension
        assert (self.s*self.h == self.emb) , "Embedding dimension must be divisible by no of heads"
        
        # the input will go through 3 differnt linear layers for different representations as
        # query, value and keys 
        
        self.values = nn.Linear(embed_dim,embed_dim,bias=False)
        self.keys = nn.Linear(embed_dim,embed_dim,bias=False)
        self.querys = nn.Linear(embed_dim,embed_dim,bias=False)
        
        # the output will go through a linear as per paper 
    ...
    def forward(self,x):
        ...
        # reshaping the dimensions for per head
        query = query.view(b,t,h,s)
        value = value.view(b,t,h,s)
        key = key.view(b,t,h,s)
        
        # prepararion for batch matrix multiplication 
        ...
        # bmm of query and key. although there is another implementation using 
        # einstein summation(einsum) which is more concise
        # I used bmm to have a good understanding of the dimensions
        # check out https://github.com/pbloem/former/issues/4 for using einsum in self attention
        # check out https://rockt.github.io/2018/04/30/einsum to know more about einsum
        
        out = torch.bmm(query, key.transpose(1,2)).view(b*h,t,t)
        
        # masking isn't needed here for encoder as the objectives of encoder doesn't require it
        # we use mask in decoder for tasks like casual lanuage modelling
        if self.mask:
            out = mask_(out,maskval='-inf',mask_diagonal=False)
        
        # scaling and the factor sqrt(dk)(from paper) i.e embedding dimensions
        # and using a soft max to add up to 1 and that gives the weights for our input sequence
        weights = nn.functional.softmax(out//(e**0.5),dim=2)
        
        # final dot product to get the attention with value
        attention = torch.bmm(weights,value).view(b,h,t,s)
        
        # concatenating the dimension as per the paper
        attention = attention.transpose(1,2).contiguous().view(b,t,h*s)
        
        # last liner layer that will give us the output
